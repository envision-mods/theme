...
function template_admin()
{
	...
	echo '
						<div id="update_section"></div>
							<div id="live_news" class="floatright descbox">
										<h2>', $txt['live'], '</h2>
								<div id="lfyi">
					...
					<script>
						var oAdminIndex = new smf_AdminIndex({
							...
							sAnnouncementMessageTemplate: ', JavaScriptEscape('
								<strong>%subject%</strong><div class="ssmalltext">%time%</div>
								<hr />
								%message%
								<div class="righttext">
	...
	$end_date = '3000-01-01';
	$context['bgtracker']['start_date'] = $start_date;
	$context['bgtracker']['end_date'] = $end_date;
	...
	$days_result = $smcFunc['db_query']('', '
		SELECT date, topics, posts, registers, most_on, hits
	...
	$d = array();
	while ($day = $smcFunc['db_fetch_assoc']($days_result))
	{
		$d['labels'][] = $day['date'];
		$d['new_topics'][] = $day['topics'];
		$d['new_posts'][] = $day['posts'];
		$d['new_members'][] = $day['registers'];
		$d['most_members_online'][] = $day['most_on'];

	//	if (!empty($modSettings['hitStats']))
			$d['hits'][] = $day['hits'];
	}
	$smcFunc['db_free_result']($days_result);
	$context['bgtracker_labels'] = array(
			'hits' => '200,180,140',
...
//			'205,151,187',
	...
	<div id="statistics" class="main_section">
		<div class="windowbg2">
			...
			<div class="content"><canvas id="myChart"></canvas></div>
	...
	echo '
	<script type="text/javascript" src="', $settings['theme_url'], '/scripts/chart.js"></script>
	<script>
		let
...
var ctx = document.getElementById("myChart");';
		...
		foreach ($context['bgtracker_labels'] as $key => $color)
			echo 'datasets.push({
				fill: false,
				label: txt["', $key, '"],
				borderWidth: 1.5,
