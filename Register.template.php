...
function template_registration_form()
{
	...
	echo '
		<form action="', !empty($modSettings['force_ssl']) ? strtr($scripturl, array('http://' => 'https://')) : $scripturl, '?action=signup2" method="post" accept-charset="', $context['character_set'], '" name="registration" id="registration" onsubmit="return verifyAgree();">
			<div class="cat_bar">
				...
				<fieldset><legend>', $txt['required_info'], '</legend>
					<div class="input-container">
						<label>', $txt['username'], '</label>
						<input required type="text" name="user" id="smf_autov_username" size="30" tabindex="', $context['tabindex']++, '" maxlength="25" value="', $context['username'] ?? '', '" />
						<label>', $txt['email'], '</label>
							<input required type="text" name="email" id="smf_autov_reserve1" size="30" tabindex="', $context['tabindex']++, '" value="', $context['email'] ?? '', '" />
						<label>', $txt['choose_pass'], '</label>
							<input required type="password" name="passwrd1" id="smf_autov_pwmain" size="30" tabindex="', $context['tabindex']++, '" />
						<label>', $txt['verify_pass'], '</label>
							<input required type="password" name="passwrd2" id="smf_autov_pwverify" size="30" tabindex="', $context['tabindex']++, '" />
					<div class="checkbox">
							<input type="checkbox" name="notify_announcements" id="notify_announcements" tabindex="', $context['tabindex']++, '"', $context['notify_announcements'] ? ' checked' : '', '>
	...
	if (!empty($context['profile_fields']) || !empty($context['custom_fields']))
		echo '
				<fieldset><legend>', $txt['additional_information'], '</legend>
					<dl class="register_form">';
	...
	if ($context['visual_verification'])
		echo '
				<fieldset><legend>', $txt['verification'], '</legend>
